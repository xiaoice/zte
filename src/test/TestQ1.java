...
{  
    ...
    public TestQ1(int qq, String password)  
    {   
        this.qq = qq;  
    ...
    {  
    	TestQ1 clinet = new TestQ1(137742685, "kill7025799");  
        try  
        {  
    ...
    {  
        ...
        sendMsg(from_uin, "此乃QQ聊天机器人程序测试中，消息会转发到QQ：137742685上");  
        Thread.sleep(2000);  
        User u = firends2.get(from_uin);  
        if(null == u)  
            sendMsgToQQ(137742685, "[qq = "+from_uin+"] send message  :\r\n"+content);  
        else  
        {  
            log("receive [qq = "+u.getQq()+" and name ="+u.getNick()+"] message {"+content+"}  ~~~");  
            sendMsgToQQ(137742685, "[qq = "+u.getQq()+" and name ="+u.getNick()+"] send message  :\r\n"+content);  
        }  
    }  
